...
contract AegylaxLens is AegylaxStorage {
    ...
    /**
     * The attack's one-time pad, once the epoch has been published.
     *
     * Empty before `revealEpochAttack`. Afterwards anybody can decrypt every
     * already-public masked point and subtract this key — that is the map.
     * Scoring still verifies the attestation and the unmask on chain.
     */
    function getMaskKey(bytes32 attackId)
        external
        view
        returns (bool published, bytes32 handle, uint256 value)
    {
        GameStorage storage $ = _gameStorage();
        published = $.maskPublished[attackId];
        handle = $.attacks[attackId].maskKeyHandle;
        if (published) value = $.publishedMaskKeys[attackId];
    }
