...
contract IncoConfidentialEngine is IConfidentialEngine, Ownable {
        ...
        returns (bytes32 bearingHandle, bytes32 deltaHandle, bytes32 maskKeyHandle)
    {
        ...
        euint256 maskKey = e.randBounded(type(uint128).max);
        ...
        return (euint256.unwrap(thetaRaw), euint256.unwrap(deltaRaw), euint256.unwrap(maskKey));
    }
    ...
    {
        euint256 masked = e.add(euint256.wrap(pointHandle), euint256.wrap(maskKeyHandle));
        e.allowThis(masked);
        // The pad is public from submit. Unmasking still wraps at 2^128.
        e.reveal(masked);
        return euint256.unwrap(masked);
    }
    ...
    function newProbeHint(
        ...
        e.allowThis(hint);
        /*
         * Granted here, in the same transaction that computes it. The delay
         * moved to `sendProbe` and `submitDefense`, which are the two calls
         * that can act on a reading — see `FhenixConfidentialEngine`.
         */
        e.allow(hint, player);
