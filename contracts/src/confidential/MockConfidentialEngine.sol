...
import {DefenseMask} from "../libraries/DefenseMask.sol";
...
contract MockConfidentialEngine is IConfidentialEngine {
        ...
        returns (bytes32 bearingHandle, bytes32 deltaHandle, bytes32 maskKeyHandle)
    {
        ...
        maskKeyHandle = keccak256(abi.encode(attackId, "mask"));
        _values[bearingHandle] = thetaRaw;
        _values[deltaHandle] = deltaRaw;
        _values[maskKeyHandle] = _draw(attackId, "mask");
    ...
    {
        maskedHandle = keccak256(abi.encode("masked", pointHandle, maskKeyHandle));
        _values[maskedHandle] = DefenseMask.mask(_values[pointHandle], _values[maskKeyHandle]);
        // The pad is public from the moment the point is accepted. `K` is not.
        unlocked[maskedHandle] = true;
    }
    ...
    function newProbeHint(
        ...
        // Named for its reader here, in the same call: the delay moved to
        // `sendProbe` and `submitDefense` (see `FhenixConfidentialEngine`).
