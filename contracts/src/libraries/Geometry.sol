 ...
 * threat is actually there. The earliest hit is the kill and takes the
 * pool; hits sharing that block share it.
 */
library Geometry {
     ...
     * waiting is not a hit. This function judges one defense and says
     * nothing about who won: ranking hits against each other is
     * `Resolution`'s job.
     */
    function evaluateDefense(
        ...
        if (entered && flightBlocks > 0) {
            // `entryT` is already 0..TIME_SCALE. Multiplying by flightBlocks
            // yields the scaled pass: launch + t * flight. Dividing by
            // TIME_SCALE a second time stored ~launch for every chord miss,
            // so an early submit was labelled TOO LATE.
            ev.interceptionBlockScaled = launchScaled + entryT * uint256(flightBlocks);
            ev.interceptX = ex;
            ev.interceptY = ey;
        }
    }
    ...
    /**
     * Half of the packed word, and the reason it is 64 rather than 128.
     *
     * A Defense Point is one *encrypted* word, and the confidential layer
     * decides how wide that word may be: CoFHE's widest encrypted integer is
     * `euint128`, and there is no wider one to move to. Packing the two
     * coordinates 128 bits apart makes a 161-bit number — nothing CoFHE can
     * hold, and the encrypt call is refused before a transaction is ever
     * built.
     *
     * 64 is not a squeeze. Both halves are world units (km x 1e6) measured
     * from the board's own corner, so both are non-negative and bounded by
     * the board: the widest deployment this protocol has run is 10,000 km
     * across, which is 34 bits. A 64-bit limb leaves thirty bits of headroom
     * on each coordinate — room for a board a billion times larger — and the
     * packed word lands at 97 bits, inside `euint128` with room to spare.
     */
    uint256 internal constant POINT_LIMB_BITS = 64;
    uint256 internal constant POINT_LIMB_MASK = (1 << POINT_LIMB_BITS) - 1;

    /**
     * Unpacks the single confidential word a defender submits.
     *
     * `x | (y << 64)`, both in wu, both non-negative because the board's
    ...
    function unpackPoint(World memory w, uint256 packed) public pure returns (Point memory p) {
        p.x = int256(packed & POINT_LIMB_MASK);
        p.y = int256(packed >> POINT_LIMB_BITS);
        if (p.x > w.widthWu) p.x = w.widthWu;
        if (p.y > w.heightWu) p.y = w.heightWu;
    }

    function packPoint(uint256 x, uint256 y) internal pure returns (uint256) {
        return (x & POINT_LIMB_MASK) | (y << POINT_LIMB_BITS);
    }
}
