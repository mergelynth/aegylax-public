 ...
 *   - `DELAY_BLOCKS` is how long a reading cannot be spent. Sending another
 *     probe before that block reverts, and so does submitting a Defense
 *     Point — so dumping an allowance in one block and locking a Defense
 *     Point in the next is not a legal line.
 *
 *     It gates the two calls that can *act* on a hint rather than the read
 *     itself. The hint is granted in the transaction that computes it, which
 *     is what keeps a Recon Probe one transaction and one trip through the
 *     confidential network's ingestion; reading early buys nothing, because
 *     both moves a reading could inform are still shut for the same three
 *     blocks.
 *
...
library ReconRules {
    ...
    /// another probe, or a Defense Point.
    uint32 internal constant DELAY_BLOCKS = 3;
