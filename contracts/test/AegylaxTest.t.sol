...
abstract contract AegylaxTest is Test {
        ...
        )
    {
        ...
        mask = GameTypes.DecryptionProof(engine.unsafePeek(attack.maskKeyHandle), noSignatures);
        ...
        for (uint256 i = 0; i < attempts.length; i++) {
            defenses[i] = GameTypes.DecryptionProof(engine.unsafePeek(attempts[i].maskedHandle), noSignatures);
        }
    }
    ...
    function completeAndReveal(bytes32 lobbyId, bytes32 attackId) internal {
        ...
        if (!lensOf().isRevealed(attackId)) {
            game.revealEpochAttack(attack.epochId, bearing, delta, mask);
        }
