...
contract GeometryUnitTest is Test {
    ...
    function test_snapshotAtSubmitIsTheIntercept() public pure {
        ...
        // The chord will pass at mid-flight. That clock has to survive so
        // the result can say TOO EARLY rather than collapsing onto launch.
        // First entry is a few blocks before closest approach — the radius
        // is crossed, not the centreline.
        assertGt(early.interceptionBlockScaled, early.arrivalBlockScaled);
        assertApproxEqAbs(
            early.interceptionBlockScaled,
            uint256(1000) * GameTypes.TIME_SCALE + (uint256(150) * GameTypes.TIME_SCALE) / 2,
            5 * GameTypes.TIME_SCALE
        );
