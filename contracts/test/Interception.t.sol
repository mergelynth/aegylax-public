...
contract InterceptionTest is AegylaxTest {
    ...
    function test_earlyArrivalNearEarth_misses() public {
        ...
        assertFalse(attempts[0].intercepted);
        assertGt(
            attempts[0].interceptionBlockScaled,
            attempts[0].arrivalBlockScaled,
            "on the path, submitted before the threat arrived - TOO EARLY, not TOO LATE"
        );
    }

    /**
     * The highest kill takes the round. Alice times a high intercept; Bob
     * times a low one. Both are on the threat at their own submit block, so
     * both are `intercepted` — but the threat Bob's circle closed on was
     * already gone, and only Alice is paid.
     */
    function test_earliestSnapshotHit_takesTheWholePool() public {
        (bytes32 lobbyId, bytes32 attackId) = startedLobby();
        ...
        GameTypes.DefenseAttempt[] memory attempts = lensOf().getDefenseAttempts(lobbyId);
        assertTrue(attempts[0].intercepted);
        assertTrue(attempts[1].intercepted, "bob's snapshot was on the threat too");
        assertTrue(attempts[0].isWinner);
        assertFalse(attempts[1].isWinner, "outranked by a higher kill");
        ...
        assertEq(outcome.interceptionBlockScaled, attempts[0].interceptionBlockScaled);

        (GameTypes.Lobby memory lobby,,) = lensOf().getLobby(lobbyId);
        assertEq(outcome.rewardPerWinner, uint256(lobby.rewardPool), "the whole pool, undivided");
    }

    /// Identical arrivals killed the same threat at the same coordinates, so
    /// there is nothing left to rank them by and the pool splits.
    ...
    /**
     * A tie is not capped at two. Any number of defenses can land in the
     * winning block, and all of them killed the same threat at the same
     * coordinates, so all of them are paid an equal share.
     */
    function test_threeWayTie_splitsAcrossEveryWinner() public {
        bytes32 lobbyId = createLobby();
        join(lobbyId, alice);
        join(lobbyId, bob);
        join(lobbyId, carol);
    ...
    /// Several timed intercepts at different altitudes: all snapshot-valid,
    /// all recorded, and only the highest one paid.
    function test_multipleInterceptors_allRecorded_highestWins() public {
        bytes32 lobbyId = createLobby();
        join(lobbyId, alice);
        join(lobbyId, bob);
        join(lobbyId, carol);
        ...
        assertEq(winners, 1, "only the earliest kill is paid");
        assertTrue(attempts[0].isWinner, "alice shot highest");

        (, GameTypes.Outcome memory outcome) = lensOf().getOutcome(lobbyId);
        assertEq(outcome.winners.length, 1);
        assertEq(outcome.winners[0], alice);
        assertEq(outcome.rewardPerWinner, uint256(lobby.rewardPool));
    }
