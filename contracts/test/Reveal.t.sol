...
contract RevealTest is AegylaxTest {
    function test_hiddenData_isUnavailableForTheWholeFlight() public {
        ...
        assertTrue(attempts[0].maskedHandle != bytes32(0));
        assertTrue(engine.unlocked(attempts[0].maskedHandle), "the pad is public from submit");
        assertFalse(engine.unlocked(attempts[0].pointHandle), "P itself stays sealed");
        (bool maskPublished,,) = lensOf().getMaskKey(attackId);
        assertFalse(maskPublished);
    }
    ...
    function test_complete_isPermissionlessAndUnlocksDecryption() public {
        ...
        assertTrue(engine.unlocked(attack.maskKeyHandle));
    }
    ...
    function test_reveal_rejectedBeforeCompletion() public {
        ...
        (GameTypes.DecryptionProof memory bearing, GameTypes.DecryptionProof memory delta, GameTypes.DecryptionProof memory mask,) =
        ...
        game.revealEpochAttack(epochId, bearing, delta, mask);
    }
    ...
    function test_reveal_rejectsWrongProofCount() public {
        ...
        vm.expectRevert(Scoring.ProofCountMismatch.selector);
    ...
    function test_reveal_isPermissionless() public {
        ...
        game.revealEpochAttack(attackOf(attackId).epochId, bearing, delta, mask);
    ...
    function test_twoCallReveal_producesTheSameResultAsFourCalls() public {
        ...
        (
            ...
            GameTypes.DecryptionProof memory mask,
            GameTypes.DecryptionProof[] memory defenses
        ) = proofsFor(lobbyId, attackId);
        game.revealAndResolve(lobbyId, bearing, delta, mask, defenses);
    ...
    /// Empty proofs on a non-empty team publish the map and leave scoring for later.
    function test_revealAndResolve_emptyProofsPublishWithoutScoring() public {
        (bytes32 lobbyId, bytes32 attackId) = startedLobby();
        timedSubmitOnTrajectory(lobbyId, attackId, alice, 400);
        ...
        game.revealAndResolve(lobbyId, bearing, delta, mask, none);

        (bool revealed,) = lensOf().getTrajectory(attackId);
        assertTrue(revealed, "the epoch is public");
        (bool published,,) = lensOf().getMaskKey(attackId);
        assertTrue(published, "the pad key is public");
        (, GameTypes.Outcome memory outcome) = lensOf().getOutcome(lobbyId);
        assertEq(outcome.resolvedAtBlock, 0, "the team is not yet scored");

        GameTypes.DecryptionProof[] memory extra = new GameTypes.DecryptionProof[](2);
        extra[0] = mask;
        extra[1] = mask;
        vm.expectRevert(Scoring.ProofCountMismatch.selector);
        game.revealAndResolve(lobbyId, bearing, delta, mask, extra);
    }

    function test_oneUnlockUnmasksEveryDefense() public {
        (bytes32 lobbyId, bytes32 attackId) = startedLobby();
        timedSubmitOnTrajectory(lobbyId, attackId, alice, 400);
        timedSubmitOnTrajectory(lobbyId, attackId, bob, 600);

        landAttack(attackId);
        revealEpoch(lobbyId, attackId);

        (,, uint256 key) = lensOf().getMaskKey(attackId);
        GameTypes.DefenseAttempt[] memory attempts = lensOf().getDefenseAttempts(lobbyId);
        assertEq(attempts.length, 2);

        for (uint256 i = 0; i < attempts.length; i++) {
            uint256 packed = DefenseMask.unmask(engine.unsafePeek(attempts[i].maskedHandle), key);
            assertEq(packed, engine.unsafePeek(attempts[i].pointHandle), "M - K recovers P");
            assertFalse(engine.unlocked(attempts[i].pointHandle));
        }
    }

    function test_proveDefenses_recrownsAnEarlierHit() public {
        (bytes32 lobbyId, bytes32 attackId) = startedLobby();
        timedSubmitOnTrajectory(lobbyId, attackId, alice, 300);
        timedSubmitOnTrajectory(lobbyId, attackId, bob, 700);
        ...
        uint32[] memory later = new uint32[](1);
        later[0] = 1;
        GameTypes.DecryptionProof[] memory one = new GameTypes.DecryptionProof[](1);
        one[0] = defenses[1];
        game.proveDefenses(lobbyId, later, one);

        (, GameTypes.Outcome memory lead) = lensOf().getOutcome(lobbyId);
        assertEq(lead.winners.length, 1);
        assertEq(lead.winners[0], bob);
        assertEq(lead.resolvedAtBlock, 0);

        uint32[] memory earlier = new uint32[](1);
        earlier[0] = 0;
        one[0] = defenses[0];
        game.proveDefenses(lobbyId, earlier, one);

        (, GameTypes.Outcome memory recrowned) = lensOf().getOutcome(lobbyId);
        assertEq(recrowned.winners.length, 1);
        assertEq(recrowned.winners[0], alice, "the earlier snapshot hit takes the crown");

        game.finalizeScoring(lobbyId);
        (, GameTypes.Outcome memory final_) = lensOf().getOutcome(lobbyId);
        (GameTypes.Lobby memory lobby,,) = lensOf().getLobby(lobbyId);
        assertTrue(final_.resolvedAtBlock != 0);
        assertEq(final_.winners[0], alice);
        assertEq(final_.rewardPerWinner, uint256(lobby.rewardPool));
    }
}
