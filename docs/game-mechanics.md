...
- Launch block = the start of the scheduled epoch; impact block = the
  ...
  by construction.
- The **launch bearing θ** is drawn ±60° off straight up
  (`MAX_LAUNCH_OFFSET_RADIANS`, matching `Geometry.sol`). The **impact** is
  θ + δ, with δ ±55° (`MAX_APPROACH_OFFSET_RADIANS` /
  `MAX_IMPACT_DELTA`). That is the order `Geometry.deriveTrajectory`
  uses: two confidential draws, then the ray. The emulator's
  `generateAttack` does the same, so a local round is not a different
  geometry.
- The **startPoint** is `rayExitFromEarth(θ)` — where that bearing leaves
  the playfield's outer edge. Two things fall out of δ staying under 90°:
  the approach is worth reading (a strictly radial attack would make
  every trajectory inferable from the impact sector alone) and the
  trajectory provably cannot clip Earth before reaching its target
  (`isApproachClearOfEarth`).
- The impact still lands on the **visible cap**: Earth's centre sits
  below the board, so a hit further round would be off the grid. θ + δ
  can sit further around than the old ±60° impact-first draw; the
  board-support check in Geometry is what keeps every generated path
  defendable.
...
once for that attack inside the confidential network. Extra sensor cells average away their
...
This is what keeps the [highest kill](#the-highest-kill-takes-the-round)
rule from having a solved answer. If the true bearing were knowable, every
round would end with everyone firing at the launch edge on the first block.
ε means the earliest shots are taken against the widest cone, so buying the
altitude costs real accuracy — which is the whole point of the dial.
...
`PROBE_DELAY_BLOCKS` (8) is how long a reading cannot be **spent**, not how
long it takes to arrive. The hint is computed *and granted to the sender* in
the `sendProbe` transaction, so a probe is one transaction and the answer
comes back with it.

What the delay gates is the next move that reading could inform, and both
are on chain so a client cannot skip either:

- another `sendProbe` from the same wallet reverts (`ProbeInFlight`);
- `submitDefense` from the same wallet reverts (`DefenseLockedByProbe`).

That is the same rule the old placement implied — a bot cannot dump an
allowance in one block and lock a Defense Point in the next — stated where
it can be enforced without charging every honest player a second
transaction, a second wallet signature and a second trip through the
confidential network's ingestion. A player who never probed is not delayed
by anything.
...
An answer is delivered **once**. `sendProbe` grants decryption to the wallet
that sent the probe, and there is no public read that returns it: the
plaintext is the private computation's output to one player, and the chain
never held it.
...
### The highest kill takes the round

There is one threat, so there is one kill. Among the snapshot hits, the
**earliest** is it, and it takes the whole pool. Arrival is the submit
block itself, so the earliest hit is also the highest one: ranking by
time and ranking by altitude are the same ranking, and the fiction and
the arithmetic agree.

Everything below that altitude closed on a threat that was already gone.
Its shot was not wrong — right point, right moment — and the record says
so, but there was nothing left to destroy, so it is paid nothing.

Ties are **by block**, and any number of defenses can share one. All of
them hit the same threat at the same coordinates, because one block is
one position along the trajectory; nothing is left to separate them, so
they all win and split the pool equally. Order within a block belongs to
the sequencer and is never allowed to decide money.

That gives the player a dial the pure "everyone who hits, splits" rule
did not have: **altitude against certainty**. Recon narrows the cone as
the flight runs, so shooting late is the accurate shot — and the one that
pays only if nobody dared higher. Shooting high is the blind shot that
takes the round outright.

Three answers come out of resolution:
...
- `isWinner` — the earliest of those hits, plus every hit sharing its
  block. The pool splits equally among them.
- `intercepted && !isWinner` — a good shot that lost the altitude race.

After reveal the Result HUD says YOU HIT for `isWinner` and **ALREADY
DOWN** for a hit that was outranked — deliberately not YOU MISSED, which
would tell that defender to fix an aim that was never wrong. A circle the
threat passed at the wrong time is TOO LATE or TOO EARLY. WINNERS is
...
The room size is the protocol `maxPlayers` at mint (**25** on live Sepolia),
frozen on that lobby — not a hardcoded 20. The Solidity field is `uint16`
(65535) and `ProtocolRules` only checks `max >= min`; 25 is the gas budget
of one `revealAndResolve`, not a type limit.
