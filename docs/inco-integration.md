...
> **An alternative layer, not the deployed one.** AEGYLAX ships on Fhenix
> CoFHE — see [fhenix-integration.md](fhenix-integration.md). Inco Lightning
> is the second implementation behind `IConfidentialEngine`, selected by
> configuration (`CONFIDENTIAL_ENGINE=inco`) rather than by code: the engine
> contract, the client gateway and their tests all build and run, and no
> deployment has used them.
>
> It is kept for two reasons. A boundary with one implementation behind it
> is a boundary nobody has checked, and a protocol whose privacy layer is a
> configuration choice is worth more than one whose privacy layer is a
> dependency. What it is not is a measured alternative: everything below is
> written from the adapter, and no live round has ever run on it.
...
not the contract owner. Inco Lightning is one of the places that secret can
be born, where reconnaissance is computed, and where a Defense Point stays
sealed.

## Why a confidential layer?
...
**is not confidential**. `FhenixConfidentialEngine` implements it too, over a
different network and with the same guarantees — the interface belongs to the
protocol, the provider to a deployment.
...
waiting is a miss. The earliest hit takes the pool; its block's ties split it.
